...
createApp({
    ...
    methods: {
        getHoursUtilizationClass(percentage) {
        if(percentage < 50) return 'text-danger fw-bold'
        if (percentage < 85) return 'text-warning';
        if (percentage >= 85) return 'text-success fw-bold';
        //if (percentage >= 50) return 'text-info';
        return 'text-muted';
    },

    //Get CSS class for progress bar based on utilization
getProgressBarClass(percentage, overtimePercentage = 0) {
    // Danger (less than 50%)
    if (percentage < 50) {
        return 'bg-danger';
    }
    
    // Under utilized (50% to less than 85%)
    if (percentage < 85) {
        return 'bg-warning';
    }
    
    // Regular capacity (85% and above)
    if (percentage >= 85) {
        return 'bg-success';
    }
...
//Calculate capacity statistics for the insights section
getCapacityStats() {
    const stats = {
        maxedOut: 0,           // Less than 50% (danger)
        underutilized: 0,      // 50% to less than 85%
        atRegularCapacity: 0   // 85% and above
    };

    this.paymentDetails.forEach(payment => {
        const regularUtil = payment.regular_utilization_percent || 0;
        
        if (regularUtil < 50) {
            stats.maxedOut++;
        } else if (regularUtil < 85) {
            stats.underutilized++;
        } else if (regularUtil >= 85) {
            stats.atRegularCapacity++;
        }
    });
    ...
    //Get overall employee capacity status with recommendations
    // getEmployeeCapacityStatus(payment) {
    //     const regularUtil = payment.regular_utilization_percent;
    //     const overtimeUtil = payment.overtime_utilization_percent;
        
    //     if (regularUtil < 50) {
    //         return {
    //             status: 'underutilized',
    //             message: 'Could work more regular hours',
    //             class: 'text-warning',
    //             icon: 'fa-arrow-up'
    //         };
    //     } else if (regularUtil >= 90 && overtimeUtil >= 90) {
    //         return {
    //             status: 'maxed_out', 
    ...
    //         };
    //     } else if (regularUtil >= 90) {
    //         return {
    //             status: 'full_regular',
    //             message: 'Regular hours full, overtime available', 
    //             class: 'text-info',
    //             icon: 'fa-info-circle'
    //         };
    //     } else {
