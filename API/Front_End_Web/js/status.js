  ...
  function updateDisplay(employees) {
    ...
    employees.forEach(emp => {
      fetchEmployeeShifts(emp.id).then(({ currShift, nextShift }) => {
        ...
        const alertIcon = hasCurrentShiftButNotWorking ? '<span style="color:orange; font-size:14px;"><i class="fa fa-exclamation-triangle text-warning me-1"></i>Should have been working - Replaced</span>' : '';
        ...
        const onLeaveIcon = emp.status === "On Leave" ? '<span style="color:red"><i class="fas fa-ban on-leave-icon"></i> </span>' : '';
